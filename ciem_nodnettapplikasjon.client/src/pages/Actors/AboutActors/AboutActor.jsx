  ...
  const renderDisplay = () => {
    if (type === 'sikkerhet') {
      ...
      return (
        <div className={styles.displayValues}>
          {Object.keys(values)
            .map((key) => `${key.charAt(0).toUpperCase() + key.slice(1)}: ${values[key]}`)
            .join(' | ')}
        </div>
      );
    }
...
export default function AboutActor() {
  ...
  const [infoItems] = useState([
    { id: 1, type: 'sikkerhet', initialValues: { omrade: 3, struktur: 2, eskalering: 1 } },
    { id: 2, type: 'default', initialValues: { evakuert: 18, gjenværende: 4, savnet: 5 } },
    ...
    <div className={styles.aboutActorContainer}>
      ...
      <div className={styles.topRow}>
        ...
        <div className={styles.topLeftColumn}>
          ...
          <div className={styles.contactPanel} onClick={() => setShowContactPopup(true)}>
            <img src={contactIcon} alt="Kontakt Icon" className={styles.contactIcon} />
            <p>Kontakt</p>
          </div>
        </div>
        ...
        <div className={styles.orgSection}>
          <div className={styles.orgHeader}>
            <div className={styles.orgHeaderLeft}>
              <img src={aboutActorIcon} alt="About Actor Icon" className={styles.orgIcon} />
          ...
          {orgEditMode ? (
            <>
              <textarea value={orgText} onChange={(e) => setOrgText(e.target.value)} />
              <div className={styles.editButtons}>
                <button className={styles.saveChangesButton} onClick={() => setOrgEditMode(false)}>
                  Bekreft
                </button>
                <button
      ...
      <div className={styles.bottomPanels}>
        ...
        <div className={styles.infoSjekkContainer}>
          <div className={styles.infoHeader}>
            <img src={checkListIcon} alt="Infosjekkliste Icon" className={styles.infoIcon} />
              ...
              {infoEditMode ? (
                <>
                  <div className={styles.infoField}>
                    ...
                    <textarea value={hendelse} onChange={(e) => setHendelse(e.target.value)} />
                  </div>
                  <div className={styles.infoField}>
                    <label>Nivå:</label>
                    <input type="text" value={nivaa} onChange={(e) => setNivaa(e.target.value)} />
                  </div>
                  <div className={styles.infoField}>
                    <label>Lokasjon:</label>
                    <input type="text" value={lokasjon} onChange={(e) => setLokasjon(e.target.value)} />
                  </div>
                  <div className={styles.editButtons}>
                    <button className={styles.saveChangesButton} onClick={() => setInfoEditMode(false)}>
                      Bekreft
                    </button>
                    <button
                      className={styles.cancelChangesButton}
          ...
          <div className={styles.kommunikasjonRow}>
            ...
            <button className={styles.timestampButton} onClick={() => setKommunikasjonTime(new Date())}>
              Oppdater
            </button>
          </div>
        </div>
        ...
        <div className={styles.underliggendePanel}>
          <div className={styles.actorsHeader}>
            <img src={actorsIcon} alt="Actors Icon" className={styles.actorsIcon} />
      ...
      {showContactPopup && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            <p>
              <strong>E-post:</strong> {actorData.contact?.email || 'N/A'}
            </p>
            <p>
              <strong>Telefon:</strong> {actorData.contact?.phone || 'N/A'}
            </p>
            <button className={styles.closePopupButton} onClick={() => setShowContactPopup(false)}>
              Lukk
            </button>
          </div>
        </div>
